...
#### vite 在生产环境中对静态资源的处理
`打包后更换目录可以在本地 启用live-server (Go Live)启动服务，如果webpack 需要配置 baseUrl: '/'才能开启`
<br/>

`打包后的静态资源为什么会有hash`
<br/>

`浏览器是有缓存机制的，只要静态资源名字不改，浏览器就 用缓存资源的`
<br/>

`浏览器刷新时，会重新发请求的，如果浏览器发现文件名称一样，会直接读取缓存`
<br/>

`所以 我们尽量避免文件一致，就会利用hash 算法（将一串字符串经过运算得到一个新的乱码的字符串）`
<br/>
